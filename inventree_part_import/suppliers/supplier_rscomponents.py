...
# The flow is:
#
#  -> RSComponents.setup(self, *, **kwargs)
#  -> RSComponents.search(self, search_term: str)
#     -> self.scrape(url) if not cached
#     -> BeautifulSoup parse
#     -> if it's a product page:
#       => scrap_product_page()
#     -> if it's search results:
#       => scrape_search_results()
#     -> if no matches return [], 0
#     -> filter, de-dupe results
#     -> if we have an exact match, return it
#     -> if we have multiple matches with identical mpn's, merge their price break data
#       => RSComponents.get_api_part()
#       -> return api_parts, len(api_parts)
#     -> else return filtered matches
#       => RSComponents.get_api_part()
#       -> return list(map(self.get_api_part, filtered_matches)), len(filtered_matches)
#
#  -> RSComponents.get_api_part()
#     -> if the part has no attributes: (came from search results)
#       => scrape_product_page()
#       -> fix supplier link
#       -> fix price breaks
#       -> finalize
...
class RSComponents(ScrapeSupplier):
    ...
    def setup(
        ...
        *,
        ...
        scraping: bool,
        browser_cookies: str = "",
        locale_url: str = "za.rs-online.com",
        **kwargs: Any,
    ):
        ...
        if browser_cookies:
            self.cookies_from_browser(browser_cookies, "za.rs-online.com")
    ...
    def search(self, search_term: str) -> tuple[list[ApiPart], int]:
        ...
        search_term_norm  = search_term_lower.replace('-', '').removeprefix('0')
        ...
        # TODO de-compose these last two steps so we only have one return
    ...
    def get_api_part(self, rs_part: dict[str, Any]) -> ApiPart:
        """ Takes a part in the supplier format and converts it to an Inventree ApiPart """

        # If the part does not have any attributes it came from the
        # search results, so we need to also scrape the product page
        # for it.
        parameters: dict[str, Any] = rs_part.get("ProductAttributes", {})
        if not len(parameters):
            ...
            try:
                ...
                if not (result := self.scrape(rs_url)):
        ...
        api_part = ApiPart(
            ...
            currency           = "ZAR", # TODO fix this rs_part.get("Currency", self.currency),
        )
...
def scrape_product_page(result, soup, search_term=None) -> dict[str, Any]:
    ...
    try:
        ...
        crumbs               = breadcrumb_container.select("div > div > div > a > span")
